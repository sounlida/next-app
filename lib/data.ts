...
export async function fetchFilteredProduct(query: string, currentPage: number) {
  const offset = (currentPage - 1) * ITEMS_PER_PAGE;
  try {
    const product = await sql<ProductItems>`
      SELECT
      product.id,
      product.title,
      product.price,
      ...
      product.title ILIKE ${`%${query}%`} OR
      product.price::TEXT ILIKE ${`%${query}%`} OR
      product.images ILIKE ${`%${query}%`} OR
      product.colors ILIKE ${`%${query}%`}
